	...
	$.ajax({
		...
		success: function (data) {
			...
			for (i = entries - 1; i >= 0; i--) {
				...
				message = msg.sender.displayname + ": " + message + " " + path;
				writeToTrace(message);
			}
		},
		error: function (error) {
...
function populateTraceRecordings() {
	$.ajax({
		headers: {
			'Authorization': 'jwt ' + token,
			'Accept': 'application/json',
		...
		url: 'https://spacesapis.avayacloud.com/api/spaces/' + spaceId + '/meetings',
		type: "GET",
		dataType: "json",
		success: function (data) {
			var message;
			for (i = data.total - 1; i >= 0; i--) {
				if (data.data[i].content.data.length > 0) {
					for (j = data.data[i].content.data.length - 1; j >= 0; j--) {
						message = "Meeting Recording:" + " recording.mov" + " " + data.data[i].content.data[j].path;
						writeToTrace(message);
					}
				}
			}
		...
		success: function (data) {
			...
			socketConnection.on('MESSAGE_SENT', function (msg) {
				...
				if (category == "chat") {
					...
					message = msg.sender.displayname + ": " + msg.content.data[0].name + " " + path;
					writeToTrace(message);
				}
			});
			...
			socketConnection.on('MEDIA_SESSION_RESPONSE', (msr) => {
				...
				if (msr.category == "app.event.recording.ready") {
					var message = "Meeting Recording:" + " recording.mov" + " " + msr.content.data[0].path;
					writeToTrace(message);
				}
