...
{
    shortenFunction(req.body.link, res);
});

async function shortenFunction(link, res) {
	var result = await callCuttly(link);
	var jsonResponse = JSON.parse(result);
	res.type('application/json');
	res.send(JSON.stringify({link:jsonResponse.url.shortLink}));	
}

async function callCuttly(link) 
{
	var url = "https://cutt.ly/api/api.php?key=98fd47177e088ea56fa69c82eae2734ac793b&short=" + encodeURI(link);
	var response = await request.get({url: url, headers : {'Accept' : 'application/json'}}, function(e , r , body) {
	});
    return response;	
}
